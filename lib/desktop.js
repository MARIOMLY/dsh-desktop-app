 ...
 * no such ambiguity.
 *
 * @param {{nodePath: string, dshBin: string, logPath: string, port?: number}} options
 * @returns {string}
 */
export function buildStartCommand(options) {
  const { nodePath, dshBin, logPath, port } = options
  const win = (p) => (IS_WINDOWS ? String(p).replace(/\//g, '\\') : String(p))
  // The port MUST be explicit. The launcher and the restart helper are generated for a
  // specific port (whatever the web server is using at the time), so a start command
  // without --port silently binds the default one instead. Field failure: the helper
  // waited for 8080 while the launcher started 3080, the helper's fallback fired, and
  // two instances fought over the port. Explicit is the only safe form here.
  const portNumber = Number(port)
  const portArg = Number.isFinite(portNumber) && portNumber > 0 ? ` --port ${Math.trunc(portNumber)}` : ''
  return [
    '@echo off',
    'rem Generated by dsh-desktop-app. Starts the DSH web server, output captured.',
    `"${win(nodePath)}" "${win(dshBin)}" web${portArg} --no-open > "${win(logPath)}" 2>&1`,
    '',
  ].join('\r\n')
}
...
# A tokenised URL must be checked against whatever is listening RIGHT NOW.
#
# DSH mints a new token on every start. If the live server was started some other way
# (a stray console running "npx @deepseek-ai/dsh web", a manual run), it writes no log,
# so the URL recorded here belongs to a dead session. Opening a window with it renders
# the UI but makes every API call fail with 401, which the user sees as "the model
# never loads" and "nothing is clickable". Verified in the field. So: adopt the running
# server only when its recorded URL actually authenticates; otherwise take the port.
function Test-UrlAlive([string]$candidate) {
  if (-not $candidate) { return $false }
  try {
    $probe = Invoke-WebRequest -Uri $candidate -UseBasicParsing -TimeoutSec 8
...
function Get-RecordedUrl {
  ...
  if ($first) { return $first.Matches[0].Groups[1].Value }
  return ''
}
...
if (Test-Listening) {
  if (Test-UrlAlive $url) {
    Write-Log 'server already listening and the recorded url is valid; opening directly'
  } else {
    Write-Log ('server already listening but the recorded url is unusable ("' + $url + '"); taking over')
    $takeOver = $true
  }
} else {
  $takeOver = $true
...
if ($takeOver) {
  if (Test-Listening) {
    # Something else owns the port and cannot be adopted (no log, or a dead token).
    # Stop it and start the server under this launcher's control.
    foreach ($owner in (Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue)) {
      Write-Log ('stopping pid ' + $owner.OwningProcess + ' that holds the port')
    ...
    Write-Log ('port released=' + (-not (Test-Listening)))
  }
  $busy = Test-Starting
  Write-Log ('starting the server; already-starting=' + $busy)
  if (-not $busy) {
  ...
  # The freshly started server writes its tokenised URL into the log.
  $url = ''
  for ($j = 0; $j -lt 60; $j++) {
    $url = Get-RecordedUrl
    if ($url) { break }
...
function writeLauncherArtifacts(ctx) {
  ...
  const startCmd = buildStartCommand({ nodePath, dshBin, logPath: SERVER_LOG_PATH, port })
...
export function actionInstall(ctx) {
  ...
  return {
    ...
    port,
    startCommand: `"${nodePath}" "${dshBin}" web --port ${port} --no-open`,
  }
}
