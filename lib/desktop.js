 ...
 * file has a UTF-8 BOM, so non-ASCII comments can corrupt parsing. Keep it
 * plain. This is a real bug we hit while building the plugin, not a style nit.
 *
 * Behaviour: if the port is already listening, just open the window; otherwise
 * start the recorded DSH command with `--no-open`, wait for the listener, read
 * the tokenised URL the server prints at boot, and open that (visiting it mints
 * the auth cookie, so the window works even with no cookie yet).
 *
 * @param {{port: number, browserPath: string, nodePath: string, dshBin: string, logPath: string}} options
 * @returns {string}
 */
export function buildLauncher(options) {
  const { port, browserPath, nodePath, dshBin, logPath } = options
  // cmd.exe runs the recorded start command, so keep Windows paths in native
  // backslash form rather than relying on forward-slash tolerance.
...
# Opens the DeepSeek Harness Web UI in a Chromium app-mode window.
$ErrorActionPreference = 'SilentlyContinue'
$ProgressPreference = 'SilentlyContinue'

$port = ${Number(port)}
$browser = ${psQuote(win(browserPath))}
$node = ${psQuote(win(nodePath))}
$dshBin = ${psQuote(win(dshBin))}
...
if (-not (Test-Listening)) {
  ...
  $cmdLine = '"' + $node + '" "' + $dshBin + '" web --no-open > "' + $log + '" 2>&1'
  Start-Process -FilePath 'cmd.exe' -WindowStyle Minimized -ArgumentList '/c', $cmdLine
  for ($i = 0; $i -lt 240; $i++) {
...
export function actionInstall(ctx) {
  ...
  const launcher = buildLauncher({ port, browserPath: browser.browser.path, nodePath, dshBin, logPath })
  if (/[^\x00-\x7F]/.test(launcher)) throw new Error('dsh-desktop-app: generated launcher is not ASCII; refusing to write it')
...
export function actionRemove(ctx) {
  ...
  if (existsSync(LAUNCHER_PATH)) {
    rmSync(LAUNCHER_PATH, { force: true })
    removed.push(LAUNCHER_PATH)
  }
