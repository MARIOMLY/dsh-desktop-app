...
function renderReport(value) {
  ...
  if (value.action === 'install') {
    lines.push(
      ...
      `  port        : ${value.port}`,
      `  start cmd   : ${value.startCommand} --no-open`,
...
export function apply(ctx) {
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      "'remove' deletes both; 'open' opens the app window right now. Windows is required for install/remove.",
    parameters: {
      action: {
        type: 'string',
        required: true,
        description: "One of: 'status', 'install', 'remove', 'open'.",
      },
      browser: {
        type: 'string',
